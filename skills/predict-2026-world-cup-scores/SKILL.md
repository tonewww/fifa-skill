...
```
   ...
   - During the knockout phase, after the base strength table is built, refresh still-alive teams with bounded tournament-performance corrections. This uses only 90-minute World Cup results, active bracket teams from completed knockout winners plus the upcoming odds JSON, and keeps corrections small so recent form updates do not erase baseline strength.

```bash
python3 skills/predict-2026-world-cup-scores/scripts/refresh_survivor_form.py --db data/worldcup2026.sqlite --rating-date 2026-07-03 --results-dir data/results --upcoming-json match/2026-07-04.json
```
...
```
   ...
   - Publishing/model rule after the 2026-07-01 and 2026-07-03 reviews: WDL direction can be correct while the score tail is still too narrow, but high-score tails should not automatically replace the higher-probability headline score. If the favorite is above roughly 52%, exact-score market prices 3-0/3-1 close to normal favorite scores, and the underdog clean-sheet/low-xG profile is weak, keep a 3-0 or 3-1 tail candidate inside displayed Top 8/Top 3 or notes. Promote it to the headline only when its blended probability remains close to the top exact score; otherwise keep the mathematically stronger 1-0/2-1/2-0 style headline and explain the tail separately.
   ...
   - Publishing/model rule after the 2026-07-04 review: the Round of 32 again showed that a WDL favorite can advance while the 90-minute score stays level. For knockout reports, if the WDL favorite is not overwhelming and the 90-minute draw probability remains material, label the relationship `主胜防平` / `客胜防平` and show both probabilities. Keep the headline score aligned with the favorite unless the raw top score is a protected draw, but make the regulation-time draw risk visible in the WDL table and analysis text.
   - Publishing/model rule after the 2026-07-05 review: when WDL direction is correct but a winner's margin is underestimated, first refresh survivor form and defensive/attacking strength from the new regulation-time result. Do not automatically promote a 3-0/0-3 tail to the headline in knockout matches unless its blended probability is close to the top score. Instead, keep high-margin winner scores visible in the full score table/notes, and let bounded survivor corrections carry the signal into the next slate.
   - Publishing/model rule after the 2026-07-06 review: weak favorites in the 45%-55% WDL band remain upset-prone in knockout regulation time. When the recent survivor correction favors the nominal underdog's attacking form or pressure profile, avoid treating the favorite label as a stable anchor; keep `防平/防冷` language, preserve BTTS/high-total alternatives in Top 8, and let the odds table's away/upset score mass influence the score tail rather than forcing all probability-first outputs into the narrow favorite result.
...
10. Analyze exact-score odds when the user provides a local odds JSON.
   ...
   - Apply parlay settlement caps when calculating and selecting multi-leg candidates: 3-leg exact-score parlays use an effective odds cap of 100,000x per stake unit; 4-leg exact-score parlays use an effective odds cap of 250,000x per stake unit. Keep the raw combined odds for transparency, but calculate hit return, expected return, expected net profit, ROI, EV sorting, cap-aware odds-first sorting, and settlement balance scores from the capped effective odds. Near the cap, prefer the higher-probability/higher-EV combination instead of chasing raw odds that no longer improve settlement return.
